...
public class String2DateUtil {
    ...
    public static String getStandardDate(String timeStr) {
        ...
        long day = (long) Math.floor(time/24/60/60/1000.0f);// 天前
         ...
         if (hour >=1) {
            if (hour >= 24) {
                ...
                sb.append(format2Datetime2(timeStr));
            }
        } else if (minute - 1 > 0) {
            if (minute >=60) {
                sb.append(format2Datetime2(timeStr));
        ...
        if (!sb.toString().equals("刚刚")&&!sb.toString().contains("/") && !sb.toString().contains(":")) {
            sb.append("前");
        }
        return sb.toString();
    }
    ...
    public static String format2Datetime2(String time) {
        Date date=new Date(Long.parseLong(time));
        return datetimeFormat3.format(date);
    }
    public static final SimpleDateFormat datetimeFormat2 = new SimpleDateFormat(
            "MM/dd");
    public static final SimpleDateFormat datetimeFormat3 = new SimpleDateFormat(
            "HH:mm");
